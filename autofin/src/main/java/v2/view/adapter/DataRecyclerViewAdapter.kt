...
class DataRecyclerViewAdapter(var dataListValue: List<DataSelectionDTO>?, itemClick: itemClickCallBack?) : RecyclerView.Adapter<DataRecyclerViewAdapter.MyViewHolder>(), Filterable {
            ...
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                if (charSearch.isEmpty()) {
                    ...
                    for (row in dataListValue!!) {
                        if (row.toString().toLowerCase().contains(constraint.toString().toLowerCase())) {
                            resultList.add(row)
                        }
                    }
